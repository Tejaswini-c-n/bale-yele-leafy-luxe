...
import servingImage from "@/assets/banana-leaf-serving.jpg";
import closeupImage from "@/assets/food-closeup.jpg";
import atmosphereImage from "@/assets/restaurant-atmosphere.jpg";
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 transition-colors ${scrolled || open ? "bg-primary/95 backdrop-blur-md" : "bg-transparent"}`}>
    <div className="mx-auto grid h-20 max-w-[1440px] grid-cols-[minmax(0,1fr)_auto] items-center px-5 sm:px-8 lg:px-12">
      <a href="#home" className="min-w-0 text-primary-foreground" aria-label="Bale Yele home">
        ...
        <span className="mt-1 block truncate text-[8px] font-bold uppercase tracking-[0.24em] text-primary-foreground/65">Taste of North Karnataka</span>
      </a>
  ...
  return <section id="home" className="grain relative flex min-h-[92svh] items-end overflow-hidden bg-primary">
    ...
    <div className="relative z-10 mx-auto w-full max-w-[1440px] px-5 pb-24 pt-36 sm:px-8 lg:px-12 lg:pb-16">
      <div className="max-w-3xl text-primary-foreground">
        <Eyebrow light>Authentic taste of North Karnataka</Eyebrow>
      ...
      <a href="#story" className="mt-14 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-primary-foreground/65">Scroll to discover <ArrowDown className="h-3.5 w-3.5" /></a>
    </div>
  </section>;
}
...
function Story() {
  return <section id="story" className="bg-background px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1320px] gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
    ...
    <div className="relative"><img src={closeupImage} alt="Close view of a vegetarian North Karnataka meal" loading="lazy" width={1008} height={1408} className="ml-auto aspect-[4/5] w-[88%] object-cover" /><div className="absolute -bottom-6 left-0 bg-accent px-6 py-5 text-accent-foreground"><span className="block font-display text-3xl">₹200–₹400</span><span className="text-[10px] font-bold uppercase tracking-[0.16em]">Approx. per person</span></div></div>
  </div></section>;
}

function Experience() {
  return <section id="experience" className="bg-primary py-24 text-primary-foreground lg:py-32"><div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12"><div className="grid gap-8 lg:grid-cols-2 lg:items-end"><div><Eyebrow light>The banana leaf experience</Eyebrow><SectionHeading light>SERVED THE WAY<br />TRADITION INTENDED.</SectionHeading></div><p className="max-w-lg pb-2 text-base leading-8 text-primary-foreground/70">Fresh, comforting and deeply rooted in tradition — experience North Karnataka meals served on a banana leaf.</p></div><div className="grain mt-14 overflow-hidden"><img src={servingImage} alt="A North Karnataka vegetarian meal being served on a banana leaf" loading="lazy" width={1600} height={1104} className="aspect-[16/8] w-full object-cover transition-transform duration-1000 hover:scale-[1.02]" /></div><div className="mt-7 grid grid-cols-2 border-y border-primary-foreground/20 sm:grid-cols-4">{["Tradition", "Flavour", "Homely", "Vegetarian"].map((item, i) => <div key={item} className="border-primary-foreground/20 px-4 py-5 text-center text-[11px] font-bold uppercase tracking-[0.2em] even:border-l sm:border-l sm:first:border-l-0"><span className="mr-2 text-accent">0{i+1}</span>{item}</div>)}</div></div></section>;
}

const menuSlots = ["Traditional Meals", "Regional Favourites", "From the Kitchen"];
function MenuPreview() {
  return <section id="menu" className="bg-background px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-[1320px]"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><Eyebrow>Menu preview</Eyebrow><SectionHeading>WHAT'S ON THE LEAF?</SectionHeading></div><p className="max-w-sm text-sm leading-7 text-muted-foreground">Our full menu is being prepared for the website. Visit or call us to discover what is currently being served.</p></div><div className="mt-14 grid border-y border-border md:grid-cols-3">{menuSlots.map((item, i) => <article key={item} className="group min-h-64 border-border p-7 transition-colors hover:bg-muted md:border-l md:first:border-l-0"><span className="text-xs font-bold text-accent">0{i+1}</span><div className="mt-20 flex items-end justify-between gap-4"><div><h3 className="text-3xl">{item}</h3><p className="mt-3 text-sm text-muted-foreground">Menu details coming soon</p></div><ArrowRight className="h-5 w-5 shrink-0 transition-transform group-hover:translate-x-1" /></div></article>)}</div><Button asChild variant="heritage" size="heritage" className="mt-10"><a href={phoneHref}>Call to enquire <Phone /></a></Button></div></section>;
}

function Gallery() {
  const tiles = [
    { src: heroImage, alt: "A complete vegetarian meal on a banana leaf", label: "The leaf", cls: "md:col-span-7 md:row-span-2" },
    { src: closeupImage, alt: "Close-up of regional vegetarian food", label: "Flavour", cls: "md:col-span-5 md:row-span-3" },
    { src: servingImage, alt: "Traditional banana leaf serving moment", label: "Tradition", cls: "md:col-span-4 md:row-span-2" },
    { src: atmosphereImage, alt: "Warm and welcoming restaurant dining atmosphere", label: "Welcome", cls: "md:col-span-3 md:row-span-2" },
  ];
...
function InstagramSection() {
  const images = [heroImage, servingImage, closeupImage, atmosphereImage, closeupImage, servingImage];
...
function Visit() {
  return <section id="visit" className="bg-accent px-5 py-24 text-accent-foreground sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1320px] gap-14 lg:grid-cols-2"><div><Eyebrow>Rajarajeshwari Nagar, Bengaluru</Eyebrow><SectionHeading>COME TASTE IT<br />FOR YOURSELF.</SectionHeading><h3 className="mt-9 text-2xl">Bale Yele – Taste of North Karnataka</h3><address className="mt-5 max-w-md not-italic leading-7">1st Floor, Kalpavruksha Building 58,<br />Opp. BMTC Bus Depot, BEML Layout, 5th Stage,<br />Rajarajeshwari Nagar, Bengaluru – 560098</address><a href={phoneHref} className="mt-6 inline-block font-display text-3xl">99168 28866</a><div className="mt-9 flex flex-wrap gap-3"><Button asChild variant="default" size="heritage"><a href={mapsHref} target="_blank" rel="noreferrer">Get directions <MapPin /></a></Button><Button asChild variant="heritageOutline" size="heritage"><a href={phoneHref}>Call now <Phone /></a></Button></div></div><a href={mapsHref} target="_blank" rel="noreferrer" className="group relative min-h-96 overflow-hidden bg-primary"><img src={atmosphereImage} alt="A warm restaurant setting for traditional banana leaf dining" loading="lazy" width={1408} height={1008} className="absolute inset-0 h-full w-full object-cover opacity-70 transition-transform duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-primary/90 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-8 text-primary-foreground"><MapPin className="h-7 w-7 text-accent" /><p className="mt-3 font-display text-3xl">Opp. BMTC Bus Depot</p><p className="mt-2 text-sm text-primary-foreground/70">Open in Google Maps</p></div></a></div></section>;
}

function Footer() { return <footer className="bg-primary px-5 pb-28 pt-20 text-primary-foreground sm:px-8 lg:px-12 lg:pb-10"><div className="mx-auto max-w-[1320px]"><div className="grid gap-12 border-b border-primary-foreground/15 pb-14 md:grid-cols-[1.5fr_1fr_1fr]"><div><p className="font-display text-5xl">BALE YELE</p><p className="mt-2 text-xs font-bold uppercase tracking-[0.22em] text-accent">Taste of North Karnataka</p></div><nav className="grid grid-cols-2 gap-3 text-sm">{nav.filter(([label]) => label !== "Experience").map(([label, href]) => <a key={href} href={href} className="text-primary-foreground/70 hover:text-primary-foreground">{label}</a>)}</nav><div className="text-sm leading-6 text-primary-foreground/70"><a href={phoneHref}>99168 28866</a><p className="mt-3">Rajarajeshwari Nagar<br />Bengaluru – 560098</p><a className="mt-3 inline-block" href={instagramHref} target="_blank" rel="noreferrer">@baleyele_taste_of_uttarkarnatk</a></div></div><p className="pt-7 text-xs text-primary-foreground/50">© 2026 Bale Yele. All rights reserved.</p></div></footer>; }
